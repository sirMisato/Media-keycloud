...
<a href="{{ route('article',$lead->slug) }}" class="lead-photo">@if($lr->coverUrl())<img src="{{ $lr->coverUrl() }}" alt="{{ $lr->cover_caption ?: 'Ilustrasi kajian pesantren' }}" fetchpriority="high">@else<div class="lead-placeholder">{{ $site['name'] }}</div>
@endif<span class="lead-badge">PILIHAN REDAKSI</span></a>
